...
/*
 This code has been borrowed from the project given at the link:
...
#define COLOURBAR_ENABLE    1
#define COLOURBAR_DISABLE   0
...
/*                          Function Defines                                                */
/********************************************************************************************/
void wrReg(unsigned char RegisterID,unsigned char Data)
{
    Wire.beginTransmission(0x21); //0x21
...
void SetColourBar(void)
{
    wrReg(0x70,0xba); //ba for colour bar
    wrReg(0x71,0xb5);//b5 for colour bar 
    wrReg(0x42,0x08);  //0x00 for normal //0x08 for colour bar*/
}

void SetVGAMonochromeCapture(void)
{
    setRes(VGA_MODE);
    setColor(YUV422_MODE);
    wrReg(0x11, 42);   // for vga monochrome
}
void SetQVGAMonochromeCapture(void)
{
    setRes(QVGA_MODE);
    setColor(YUV422_MODE);
    wrReg(0x11, 17);   // for qvga monochrome
}
void SetQQVGAMonochromeCapture(void)
{
    setRes(QQVGA_MODE);
    setColor(YUV422_MODE);
    wrReg(0x11, 17);   // for qqvga monochrome
}
void SetQQVGAColourImageYUVCapture(int colourbarenable)
{
    setRes(QQVGA_MODE);
    setColor(YUV422_MODE);
    wrReg(0x11, 42); //for qqvga with y and uv colour
...
void SetQQVGAColourImageRGBCapture(int colourbarenable)
{
    setRes(QQVGA_MODE);
    setColor(RGB565_MODE);
    wrReg(0x11, 42); //for qqvga with y and uv colour
    if(colourbarenable)
    {
        SetColourBar();
...
void setup()
{
    arduinoUnoInit();
    camInit();
    SetQQVGAColourImageYUVCapture(COLOURBAR_DISABLE);
    Wire.end();  
